...
import { Grid, Loader2, Star, Play } from 'lucide-react';
...
export const MemoryGame: React.FC<Props> = ({ levelConfig, onBack, onFinish }) => {
  ...
  const finishGame = async () => {
    ...
    // Simple scoring: 100 base, minus 10 per extra move
    ...
    const msg = await generateEncouragement(rawScore, "麻将对对碰");
    setMessage(msg);
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-6 space-y-8 animate-fade-in bg-gradient-to-br from-rose-50 to-pink-100">
        <div className="text-center bg-white p-8 rounded-3xl shadow-xl max-w-sm w-full border-4 border-white">
          ...
          <p className="text-xl text-slate-600 leading-relaxed">翻开牌面，<br/>寻找相同的麻将牌。</p>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl border-4 border-rose-100 max-w-md w-full relative overflow-hidden">
          ...
          <div className="flex flex-col gap-4">
            <button 
              ...
              className="bg-rose-600 text-white text-2xl py-4 rounded-2xl shadow hover:bg-rose-700 font-bold active:scale-95 transition-all"
            >
              {stars >= 2 ? "下一关" : "完成"}
            </button>
          ...
          <button
            ...
            className={`aspect-[3/4] rounded-xl shadow-md flex items-center justify-center transition-all duration-500 transform perspective-1000
              ${card.isFlipped || card.isMatched 
                ? 'bg-orange-50 border-4 border-orange-100 rotate-y-180' // 牌面颜色(米黄)
                : 'bg-gradient-to-br from-blue-600 to-blue-800 border-2 border-white'} // 牌背颜色
            `}
